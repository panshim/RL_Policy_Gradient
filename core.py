...
# https://github.com/aravindr93/mjrl/blob/master/mjrl/samplers/core.py
# The modifications are that we implemented extra sampling methods
# that can be used to sample data with mixture policy and LSTM policy.
...
def do_rollout_mixture(
    ...
    for ep in range(num_traj):
        ...
        if p < beta:
            ...
            sampling_policy = old_policy
            
        while t < horizon and done != True:
            a, agent_info = sampling_policy.get_action(o)
...
def sample_paths_mixture(
    ...
    if num_cpu == 1:
        input_dict = dict(num_traj=num_traj, env=env, policy=policy,
                          old_policy=old_policy, beta=beta,
                          eval_mode=eval_mode, horizon=horizon, base_seed=base_seed,
                          env_kwargs=env_kwargs)
        # dont invoke multiprocessing if not necessary
        return do_rollout_mixture(**input_dict)
    ...
    results = _try_multiprocess(do_rollout_mixture, input_dict_list,
                                num_cpu, max_process_time, max_timeouts)
    paths = []
    # result is a paths type and results is list of paths
    for result in results:
...
def do_rollout_lstm(
    ...
    for ep in range(num_traj):
        ...
        while t < horizon and done != True:
            aa, h, c = policy.get_action(o, h ,c)
            a, agent_info = aa
            if eval_mode:
                a = agent_info['evaluation']
            env_info_base = env.get_env_infos()
...
def sample_paths_lstm(
    ...
    if num_cpu == 1:
        input_dict = dict(num_traj=num_traj, env=env, policy=policy,
                          ...
                          env_kwargs=env_kwargs)
        # dont invoke multiprocessing if not necessary
        return do_rollout_lstm(**input_dict)
    ...
    results = _try_multiprocess(do_rollout_lstm, input_dict_list,
                                num_cpu, max_process_time, max_timeouts)
    paths = []
    # result is a paths type and results is list of paths
    for result in results:
