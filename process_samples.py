...
# https://github.com/aravindr93/mjrl/blob/master/mjrl/utils/process_samples.py
# The modification is that we added a newly implemented method used to
# compute TD targets as returns
...
def compute_returns_td(paths, gamma, baseline):
    for path in paths:
        b = baseline.predict(path)
        if b.ndim == 1:
            b1 = np.append(b, 0.0 if path["terminated"] else b[-1])
        else:
            b1 = np.vstack((b, np.zeros(b.shape[1]) if path["terminated"] else b[-1]))
        path["returns"] = path["rewards"] + gamma * b1[1:]
