# This is a newly implemented scripts, with reference to:
# https://github.com/aravindr93/mjrl/blob/master/mjrl/algos/dapg.py
# It is an implementation of simple off-policy DAPG. The only modification
# we made, compared with the reference codes, is storing the old policy and using
# the old policy to sample a small portion of data for online training.
...
class DAgger(NPG):
    def __init__(self, env, policy, old_policy, baseline,
                 demo_paths=None,
        ...
        #    process_samples.compute_returns(demo_paths, 0.995)
        #    print("Pre-training baseline ...")
        #    self.baseline.fit(demo_paths)
    ...
    def train_step(self, N,
        ...
        if sample_mode == 'trajectories':
            input_dict = dict(num_traj=N, env=env, policy=self.policy, old_policy=self.old_policy, \
                              beta=self.beta, horizon=horizon, \
                              base_seed=self.seed, num_cpu=num_cpu, env_kwargs=env_kwargs)
            paths = trajectory_sampler.sample_paths_mixture(**input_dict)
        elif sample_mode == 'samples':
            raise ValueError("DAgger only supports for sampling trajectories.")
        ...
        #else:
        #    process_samples.compute_returns_td(paths, gamma, self.baseline)
        process_samples.compute_returns(paths, gamma)
