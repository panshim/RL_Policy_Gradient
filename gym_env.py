...
# https://github.com/aravindr93/mjrl/blob/master/mjrl/utils/gym_env.py
# The only modification is that we made the visualization of policy to be
...
class GymEnv(object):
    ...
    def visualize_policy_lstm(self, policy, horizon=1000, num_episodes=1, mode='exploration'):
        try:
            self.env.env.visualize_policy_lstm(policy, horizon, num_episodes, mode)
        except:
            for ep in range(num_episodes):
                o = self.reset()
                d = False
                ...
                while t < horizon and d is False:
                    ...
                    a = aa[0] if mode == 'exploration' else aa[1]
                    o, r, d, _ = self.step(a)
                    self.render()
                    t = t+1
    ...
    def evaluate_policy_lstm(self, policy,
                             num_episodes=5,
                             horizon=None,
                             gamma=1,
                             visual=False,
