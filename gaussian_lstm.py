...
# The modification from the original reference codes is that we changed the network
# for policy to be LSTM, and that we changed the format of observation data so that
# it can be consumed by our self-implemented LSTM module.

import numpy as np
from mjrl.utils.rnn_network import RNNetwork
import torch
...
class LSTM:
    def __init__(self, env_spec,
        ...
        # Policy network
        # ------------------------
        self.model = RNNetwork(self.n, self.m, hidden_sizes)
        # make weights small
        ...
        # Old Policy network
        # ------------------------
        self.old_model = RNNetwork(self.n, self.m, hidden_sizes)
    ...
    # Main functions
    # ============================================
    def get_action(self, observation, h, c):
        o = np.float32(observation.reshape(1, -1))
        self.obs_var.data = torch.from_numpy(o)
        a, h, c = self.model.run_sequence(self.obs_var, h, c)
        mean = a.data.numpy().ravel()
        noise = np.exp(self.log_std_val) * np.random.randn(self.m)
        action = mean + noise
        return [action, {'mean': mean, 'log_std': self.log_std_val, 'evaluation': mean}], h, c
    ...
    def mean_LL(self, observations, actions, model=None, log_std=None):
        ...
        if type(actions) is not torch.Tensor:
            ...
            act_var = actions
        mean = torch.empty((0, 24))
        for ob in observations:
            obs_var = [ob]
            aa = model(obs_var)
            mean = torch.cat((mean, aa))
